...
func NewConnection(config Config) (Connection, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "Validate ssh connection parameters")
	}
	...
	endpoints := net.JoinHostPort(config.Address, strconv.Itoa(config.Port))

	client, err := ssh.Dial("tcp", endpoints, sshConfig)
	if err != nil {
		return nil, errors.Wrapf(err, "Unable to establish ssh connection: %s", endpoints)
	}
	...
	conn, err := client.Dial("tcp", endpoints)
	if err != nil {
		return nil, errors.Wrapf(err, "Unable not establish connection to %s", endpoints)
	}
	sshConfig.User = config.Username
	ncc, chans, reqs, err := ssh.NewClientConn(conn, endpoints, sshConfig)
	if err != nil {
		return nil, errors.Wrapf(err, "Unable not establish connection to %s", endpoints)
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Create sftp client failed: %v", err)
	}
...
func validateConfig(config Config) (Config, error) {
	if len(config.Username) == 0 {
		return config, errors.New("Without specifying username for ssh connection")
	}
	return config, nil
...
func (c *connection) PExec(cmd string, stdin io.Reader, stdout, stderr io.Writer, host Host) (code int, err error) {
	sess, err := c.session()
	if err != nil {
		return 1, errors.Wrap(err, "Failed to create session")
	}
	defer sess.Close()
...
func (c *connection) Exec(cmd string, host Host) (stdout string, code int, err error) {
	...
	outString := strings.TrimPrefix(string(output), fmt.Sprintf("[sudo] password for %s:", host.GetUser()))

	return strings.TrimSpace(outString), exitCode, errors.Wrapf(err, "Failed to exec command: %s \n%s", cmd, strings.TrimSpace(outString))
}
...
func (c *connection) Scp(src, dst string, host Host) error {
	...
	if err := c.MkDirAll(remotePath, "777", host); err != nil {
		return err
	}
	f, err := os.Stat(src)
	if err != nil {
